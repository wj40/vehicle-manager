...
export function ComboboxSearch({vehicles, onSelect}: Props) {
  ...
  function score(v: Vehicle): number {
    const fields = [
      ...
      { val: v.brand_name ?? "", w: 2 },
      { val: v.model_name ?? "", w: 1 },
    ]
    let s = 0
  ...
  ? filtered.map(v => `${v.brand_name ?? v.brand_id} ${v.model_name ?? v.model_id} (${v.reg_number})`)
  : []

  const lookup = useMemo(() => {
  const map = new Map<string, Vehicle>()
  vehicles.forEach(v => map.set(`${v.brand_name ?? v.brand_id} ${v.model_name ?? v.model_id} (${v.reg_number})`, v))
  return map
  }, [vehicles])
